...
class KrakenAPI:
    ...
    def __init__(self, api_key, api_secret, paper_mode: bool = False, paper_initial_balance_eur: float = 100.0):
        """If `paper_mode` is True, orders are simulated locally and no
        live AddOrder/CancelOrder requests are sent. Useful for dry‑run
        diagnostics.

        paper_initial_balance_eur: starting EUR balance used to answer
        get_account_balance() while in paper mode, since there is no real
        Kraken account to query (blank credentials would fail anyway).
        Defaults to 100.0; pass the same value as your paper config's
        [bot_settings.trade_amounts].initial_balance so reported balances
        stay consistent with what you configured.
        """
        ...
        self._paper_balance_eur = float(paper_initial_balance_eur)
    ...
    def get_account_balance(self):
        ...
        In paper mode, returns a simulated EUR-only balance instead of calling
        Kraken (there is no real account to query; the bot's own
        purchase_prices/position_qty dicts are the source of truth for
        simulated crypto holdings in this mode, not this balance dict).
        """
        if self.paper_mode:
    ...
    def place_order(self, pair, direction, volume, price=None, leverage=None, post_only=False, reduce_only=False):
        try:
            ...
            if reduce_only:
                if self.paper_mode:
                    # In paper mode, short positions are tracked in-memory via
                    # TradingBot.short_qty, not on a real Kraken account.
                    # The OpenPositions check below would always return empty
                    # (blank credentials), blocking every short close.
                    # Trust the volume passed in directly -- the caller
                    # (execute_close_short_order) already computed it from
                    # self.short_qty[pair], which is the authoritative source.
                    pass
                else:
    ...
    def get_open_orders(self):
        ...
        In paper mode, always returns an empty result: place_order() simulates
        immediate fills and never creates a real resting order on Kraken, so
    ...
    def cancel_order(self, order_id):
        if self.paper_mode:
            self.logger.info(f"[PAPER] Skipping real CancelOrder for {order_id} (paper_mode)")
            return {'simulated': True, 'count': 1}
        try:
            time.sleep(self.rate_limit_delay)
    ...
    def get_ledgers(self, asset=None, start=None, fetch_all=False, max_pages=200):
        ...
        In paper mode, returns an empty ledger: there's no real account
        behind paper_mode, so reconciliation against real deposits/
        withdrawals/fees doesn't apply.
        """
        if self.paper_mode:
            return {}
        try:
    ...
    def get_trade_history(self, start=None, fetch_all=False, max_pages=200):
        """In paper mode, returns an empty trade history. Real fill/trade
        records for paper sessions live in trade_events.jsonl (written by
        trading_bot.py itself), not on Kraken's real TradesHistory endpoint.
        """
        if self.paper_mode:
            return {}
        try:
