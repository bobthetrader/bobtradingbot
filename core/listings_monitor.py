...
def is_expired(entry: dict, hold_hours: int = 12) -> bool:
    ...
    if not buy_ts:
        # Never bought — measure from detected_at (when bot first found it on Kraken).
        # listed_at can be the blog announcement time, hours before the coin went live,
        # so using it would prematurely expire valid new listings.
